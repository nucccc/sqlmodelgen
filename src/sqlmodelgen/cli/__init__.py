...
from sqlmodelgen import gen_code_from_sql, gen_code_from_sqlite, __version__
...
def main_cli(args: Iterable[str] | None = None):
    ...
    _act_on_args(parsed, list(args), parser)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--add-header',
        action='store_true',
        help='Adds a little `generated by: <command-line-arguments> header to your files. Also include sa timestamp and version-number.',
    )

    return parser


def _act_on_args(args: argparse.Namespace, argv: list[str], arg_parser: argparse.ArgumentParser):
    # Validation: --mysql requires --dbname
    if args.mysql and not args.dbname:
        _exit('--dbname is required when using --mysql', arg_parser)
    ...
    if args.file:
        ...
        _exit('No input source specified', arg_parser)
    ...
    if args.add_header:
        ...
        output = '\n'.join(
            [
                f'# Generated by: sqlmodelgen@{__version__}',
                # Tweaks argv so `/Users/user/...etc/sqlmodelgen` is not shown
                f'# command: {shlex.join(['sqlmodelgen'] + argv[1:])}',
                f'# timestamp: {datetime.datetime.now().astimezone(timezone.utc).isoformat(timespec="seconds")}',
                output,
            ]
        )
...
def _exit(error: str, usage: argparse.ArgumentParser | str | None = None):
    if usage:
        print(usage)
    sys.exit(error)
